...
async function fetchGithubRuns() {
    ...
    try {
        ...
        data.workflow_runs.forEach(run => {
            ...
            const time = new Date(run.created_at).toLocaleString('de-DE', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' });
            
            card.innerHTML = `
                <div style="font-weight:bold; color:var(--accent);">${run.name}</div>
                <div style="margin: 4px 0;">Status: <span class="badge ${run.status}">${run.status}</span></div>
                <div style="color:var(--text-muted); font-size:10px;">Gestartet: ${time}</div>
            `;
            container.appendChild(card);
        });
    } catch (e) { container.innerHTML = 'Fehler beim Laden der GitHub-Runs.'; }
}
